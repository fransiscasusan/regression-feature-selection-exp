...
def run_OLST(X,y,true_beta,k,rho,min_offline_errors):
    ...
    for i in range(n):
        ...
        X_current, y_current = X[:i+1,:], y[:i+1]
        mu_x = np.mean(X_current,axis=0)
        mu_y = np.mean(y_current)

        S_xx = np.dot(X_current.T,X_current)*1.0/(i+1)        
        S_xy = np.dot(X_current.T,y_current)*1.0/(i+1)
    
        # Normalization
        if i>=1:
            Pi = np.linalg.inv(np.sqrt(np.multiply((S_xx - np.square(mu_x)),np.identity(p))))
            S_xx = np.dot(Pi, S_xx-np.dot(mu_x,mu_x.T))
            S_xx = np.dot(S_xx,Pi)

            S_xy = np.dot(Pi, S_xy) - mu_y*np.dot(Pi, mu_x).reshape((p,1))
        ...
        selected_X = X_current[:,k_biggest_indices]
...
if __name__ == "__main__":
    ...
    X               = generateX(100000,rho,1000)
    true_beta       = generateBeta(1000,k,sig)
    y               = generateY(X,true_beta)

    NS              = [1000,3000,10000,50000,100000]
